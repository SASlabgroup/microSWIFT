...
def add_2_send_queue(q, lq, workc):
    ...
    while ret != "done":
        ...
        if send_sensor_type == 0:
            ...
            successful_send = True # Oct '24 send_microSWIFT_50(payload_data, next_start)
        elif send_sensor_type == 51:
            successful_send = True # Oct '24 send_microSWIFT_51(payload_data, next_start)
        elif send_sensor_type == 52:
            successful_send = True # Oct '24 send_microSWIFT_52(payload_data, next_start)
        else:
...
if __name__=="__main__":
    ...
    sendqueue = Queue()
    # RS Oct '24 calcqueue = Queue()
    
    sp = Process(target=add_2_send_queue, args=(sendqueue, logqueue, worker_configurer))
    # RS Oct '24 cp = Process(target=make_calculation, args=(calcqueue, sendqueue, logqueue, worker_configurer))
    sp.start()
    # Oct '24 cp.start()
    ...
    while True:
        ...
        if recording_complete == True:
            # we don't need to calculate summary
