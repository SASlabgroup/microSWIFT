...
static opts::options_description sample_options_desc(prog_options & options)
{
  opts::options_description desc("Sample options");
  desc.add_options()
    ("sample",
      opts::value<long>(&options.samples)->default_value(100),
     "number of samples")
    ("freq_int",
      opts::value<uint16_t>(&options.frequency)->default_value(0b0100),
     "chip settings for frequency of samples (in hex):\n"
     "  1000 == 8 ( 1.66 kHz (high performance))\n"
     "  0111 == 7 (   833 Hz (high performance))\n"
     "  0110 == 6 (   416 Hz (high performance))\n"
     "  0101 == 5 (   208 Hz (high performance))\n"
     "  0100 == 4 (   104 Hz (high performance))\n"
     "  0011 == 3 (    52 Hz (high performance))\n"
     "  0010 == 2 (    26 Hz (high performance))\n"
     "  0001 == 1 (   12_5 Hz (high performance))\n")
    ;
  return desc;
}
...
static opts::options_description processing_options_desc(prog_options & options)
{
  ...
  desc.add_options()
    ("mode",
      opts::value<std::string>(&options.mode)->default_value("raw"),
    ...
    ("output_file",
      opts::value<std::string>(&options.output_file)->default_value("outfile"),
     "specifies name of the output file for data.\n")
    ;
  return desc;
}
...
static opts::options_description command_line_options_desc(prog_options & options)
{
  return general_options_desc()
    ...
    .add(sample_options_desc(options))
